...
export default function ChangePassword() {
  ...
  const [formLogin, setFormLogin] = useState(localStorage.getItem("formLogin") || "");
  ...
  const handleInputFormLogin = (event) => {
    localStorage.setItem("formLogin", event.target.value);
    setFormLogin(event.target.value);
  };
  ...
  return (
    <>
      <HeaderApp text={`Kim`} textTwo={`Yana`} />
      ...
      <div className={styles.container}>
        <form className={styles.formContainer} onSubmit={handleSubmitForm}>
          ...
          <div className={styles.inputBlock}>
            <span className={`${styles.inputs} ${styles.asteriks}`}>
              <input className={styles.input}
                type="text"
                placeholder="Email/Логин"
                value={formLogin}
                onInput={handleInputFormLogin} />
            </span>
